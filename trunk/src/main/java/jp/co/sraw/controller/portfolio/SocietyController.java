...
/**
 * <B>SocietyControllerクラス</B>
 * <P>
 * Controllerのメソッドを提供する
 */
@SuppressWarnings({ "rawtypes", "hiding" })
@Controller
@RequestMapping("/portfolio/society")
public class SocietyController extends PortfolioController {

	private static final LoggerWrapper logger = LoggerWrapperFactory.getLogger(SocietyController.class);

	@PostConstruct
	protected void init() {
		logger.setMessageSource(messageSource);
		LIST_PAGE = "portfolio/society/list";
		EDIT_PAGE = "portfolio/society/edit";
		REDIRECT_LIST = "redirect:/portfolio/society/";
	...
	@Autowired
	private SocietyServiceImpl serviceImpl;

	@Autowired
	private SocietyExcelHelper execlHelper;

	@Autowired
	private PortfolioEngine<SocietyController, GySocietyForm, SocietyServiceImpl, SocietyExcelHelper> engine;
	 ...
	 */
	@RequestMapping({ "", "/", "/list" })
	public String list(@ModelAttribute(FORM_NAME) GySocietyForm form, Model model, Locale locale) {
	...
	public String edit(@ModelAttribute(FORM_NAME) GySocietyForm form, Model model, Locale locale) {
	...
	public String copy(@Validated @ModelAttribute(FORM_NAME) GySocietyForm form, Model model, Locale locale) {
	...
	public String update(@Validated @ModelAttribute(FORM_NAME) final GySocietyForm form, BindingResult bindingResult,
	...
	public String updateAll(@Validated @ModelAttribute(FORM_NAME) final GySocietyForm form, BindingResult bindingResult,
			Model model, RedirectAttributes attributes, Locale locale) {
	...
	@ResponseBody
	public Resource exportExcel(@ModelAttribute(FORM_NAME) final GySocietyForm form,
	...
	public String importExcel(@ModelAttribute(FORM_NAME) final GySocietyForm form,
	...
	public String delete(@ModelAttribute(FORM_NAME) final GySocietyForm form, Model model,
