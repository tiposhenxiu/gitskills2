...
public class CareerController extends PortfolioController {

	private static final LoggerWrapper logger = LoggerWrapperFactory.getLogger(CareerController.class);

	@PostConstruct
	protected void init() {
		logger.setMessageSource(messageSource);
		LIST_PAGE = "portfolio/career/list";
		EDIT_PAGE = "portfolio/career/edit";
		REDIRECT_LIST = "redirect:/portfolio/career/";
		OP_FUNCID = CommonConst.OP_FUNC_USER_CAREER_CARRER;
	}
	...
	@Autowired
	private CareerServiceImpl serviceImpl;

	@Autowired
	private CareerExcelHelper execlHelper;

	@Autowired
	private PortfolioEngine<CareerController, GyCareerForm, CareerServiceImpl, CareerExcelHelper> engine;
	 ...
	 */
	@RequestMapping({ "", "/", "/list" })
	public String list(@ModelAttribute(FORM_NAME) GyCareerForm form, Model model, Locale locale) {
	...
	public String edit(@ModelAttribute(FORM_NAME) GyCareerForm form, Model model, Locale locale) {
	...
	public String copy(@Validated @ModelAttribute(FORM_NAME) GyCareerForm form, Model model, Locale locale) {
	...
	public String update(@Validated @ModelAttribute(FORM_NAME) final GyCareerForm form, BindingResult bindingResult,
	...
	public String delete(@ModelAttribute(FORM_NAME) final GyCareerForm form, Model model, RedirectAttributes attributes,
			Locale locale) {
	...
	public String updateAll(@Validated @ModelAttribute(FORM_NAME) final GyCareerForm form, BindingResult bindingResult,
			Model model, RedirectAttributes attributes, Locale locale) {
	...
	@ResponseBody
	public Resource exportExcel(@ModelAttribute(FORM_NAME) final GyCareerForm form,
	...
	public String importExcel(@ModelAttribute(FORM_NAME) final GyCareerForm form,
