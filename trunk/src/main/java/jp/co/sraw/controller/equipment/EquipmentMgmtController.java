/*
* ファイル名：EquipmentMgmtController.java
*
...
/**
 * <B>EquipmentMgmtControllerクラス</B>
 * <P>
 * Controllerのメソッドを提供する
 */
@Controller
@RequestMapping(CommonConst.PATH_MGMT+"/equipment")
public class EquipmentMgmtController extends CommonController {

	private static final LoggerWrapper logger = LoggerWrapperFactory.getLogger(EquipmentMgmtController.class);

	@Autowired
	private SupportServiceImpl supportServiceImpl; // 支援制度と共通

	private static final String REDIRECT_LIST = "redirect:list";
	private static final String LIST_PAGE = "equipment/mgmt/list";
	private static final String EDIT_PAGE = "equipment/mgmt/edit";
	...
	private static final String HIRAKU_DEFAULT ="1"; // 初期値: HIRAKUリンク集
	private static final String AREA_DEFAULT ="00"; // 初期値: 未設定
	...
	public String list(@ModelAttribute(CommonConst.FORM_NAME) SupportForm form, Model model, Locale locale) {
		...
		List<MsCodeDto> hirakuList = DbUtil.getJosuList(HIRAKU_KBN, locale);
	...
	public String edit(@ModelAttribute(CommonConst.FORM_NAME) SupportForm form, Model model, Locale locale) {
		...
		if (CommonConst.PAGE_MODE_EDIT.equals(form.getPageMode()) && StringUtil.isNotNull(form.getSupportKey())) {
			...
			if (spSupportTbl == null) {
				...
				form.setSupportAreaKbn(spSupportTbl.getSupportAreaKbn());
		...
		if (CommonConst.PAGE_MODE_ADD.equals(form.getPageMode())) {
			...
			String searchHirakuKbn = form.getSearchHirakuKbn();
			String searchAreaKbn = form.getSearchAreaKbn();
			...
			createForm.setSearchHirakuKbn(searchHirakuKbn);
			createForm.setSearchAreaKbn(searchAreaKbn);

			// 初期データ
			createForm.setSupportSpkikiKbn(KBN);
			createForm.setPublicFlag(PUBLIC_FLAG);
			createForm.setSupportHirakuKbn(HIRAKU_DEFAULT);
			createForm.setSupportAreaKbn(AREA_DEFAULT);
	...
	public String create(@Validated @ModelAttribute(CommonConst.FORM_NAME) final SupportForm form,
		...
		if (supportServiceImpl.update(form, userInfo)) {
			...
			if (CommonConst.PAGE_MODE_EDIT.equals(form.getPageMode())) {
				...
				super.operationHistory(CommonConst.OP_FUNC_SUPPORT_FACILITY, CommonConst.OP_ACTION_INSERT); // 操作ログ保存
			...
			if (CommonConst.PAGE_MODE_ADD.equals(form.getPageMode())) {
				...
				super.operationHistory(CommonConst.OP_FUNC_SUPPORT_FACILITY, CommonConst.OP_ACTION_UPDATE); // 操作ログ保存
	...
	public String delete(@ModelAttribute(CommonConst.FORM_NAME) final SupportForm form, Model model,
		...
		if (supportServiceImpl.delete(form)) {
			...
			super.operationHistory(CommonConst.OP_FUNC_SUPPORT_FACILITY, CommonConst.OP_ACTION_DELETE); // 操作ログ保存
